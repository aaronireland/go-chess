...
func TestAlgebraicToCartesian(t *testing.T) {
	for x := 0; x < 8; x++ {
		for y := 0; y < 8; y++ {
			bit := y*8 + x
			p := BitToAlgebraic(bit)
			i, j := AlgebraicToCartesian(p)
			if (i != x) || (j != y) {
